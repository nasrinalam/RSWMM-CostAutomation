...
ParametersBound<-function(ParametersFile){
  Bounds = read.csv(file=ParametersFile, header = TRUE, sep = ",", quote="\"", dec=".",
                    fill = TRUE, comment.char="",stringsAsFactors = FALSE)
  Negative.5 = Bounds$Negative.5
  Negative.10 = Bounds$Negative.10
  Negative.15 = Bounds$Negative.15
  Negative.20 = Bounds$Negative.20
  Positive.5 = Bounds$Positive.5
  Positive.10 = Bounds$Positive.10
  Positive.15 = Bounds$Positive.15
  Positive.20 = Bounds$Positive.20
  initial = Bounds$Initial
  
  return(Bounds)
}
Objectivefunction<-function(SWMMOptFile,x,OutFile,swmm){
...
Q = getSWMMTimeSeriesData(headObj,iType = 1,nameInOutputFile = "0",vIndex = 4)
SimulatedData = SimulatedData(Q,headObj)
sum2 = DurationCurves(SimulatedData)
return (sum2)
}
#Aggregating 15 min runoff
Removal<-function(sum1,sum2){
  removal = ((sum2$Sim-sum1$Sim)/sum1$Sim)*100
  return(removal)
}
